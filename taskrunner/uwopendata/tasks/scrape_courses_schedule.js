...
module.exports = async (options) => {
    ...
    options = Object.assign({
        docPath: `${approot}/uwopendata/data/courses_array.json`,
        docEncoding: 'utf8',
        batchSize: 300,
        batchDelay: 500,
    }, options);
    ...
    // Check if docPath exists
    try {
        courses_list = JSON.parse(await fs.promises.readFile(options.docPath, options.docEncoding));
    }
    catch (err) {
        if (err.code === 'ENOENT') {
            logger.verbose(`${options.docPath} does not exist`);
